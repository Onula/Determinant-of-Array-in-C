...
<p>for n x n array with n > 3 there are the following functions</p> 
  ...
  <p>&emsp;◻️CheckAndSwap() function is inside the GaussianElimination() function because if some element of diagonal is 0 needs to swap the ij row with another that the element is different from 0 (!=0) in ij-column  AND changing the sign of determinant if swap.</p>
