...
double det(int s, double array[s][s]){
	...
	if(s == 2){//2x2 array (Leibniz formula)
		...
		int sameElementsR;//The number of same elements of i-row with j-row
		int sameElementsC;// The number of same elements of i-column with j-column
		for(i=0; i<s-1; i++){    
			for(j=i+1; j<s; j++){
		...
		//Print the final version of Upper triangular array(matrix) 
        for(i=0; i<s; i++){
            printf("\n");
            for(j=0; j<s; j++){
                printf("%lf ",array[i][j]);
